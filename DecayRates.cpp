...
FormFactor::FormFactor(Grid & Lattice, vector<double> & density, int infinity) : lattice(25000, Lattice.R(0), Lattice.R(infinity), "linear")
{
	iDensity.clear();
	iDensity.resize(lattice.size());

	vector<double> ini_latt(Lattice.size(), 0);
	for (int i = 0; i < ini_latt.size(); i++) ini_latt[i] = Lattice.R(i);

	Interpolation W(6);
	vector<double> tmp(2, 0);
	for (int i = 0; i < lattice.size(); i++) {
		tmp = W.get_value(density, ini_latt, lattice.R(i));
		iDensity[i] = tmp[0];
	}

	Adams I(Lattice, 10);
	Q0 = I.Integrate(&density, 0, infinity);

	for (int i = 0; i < 50; i++)
	{
		Q_mesh.push_back(i * 0.2);
	}
}



double FormFactor::getFF(double Q)
{
	double Result = 0;
	if (Q < 0) return Result;
	if (Q == 0) return Q0;

	vector<double> integrand(lattice.size()+1, 0);

	for (int i = 0; i < lattice.size(); i++)
	{
		integrand[i] = iDensity[i]*sin(Q*lattice.R(i)) / Q / lattice.R(i);
	}

	Adams I(lattice, 10);

	Result = I.Integrate(&integrand, 0, lattice.size()-1);
	return Result;
}



vector<double> FormFactor::getAllFF()
{
	vector<double> Result(Q_mesh.size(), 0);
	vector<double> integrand(lattice.size() + 1, 0);
	Adams I(lattice, 10);

	for (int s = 0; s < Q_mesh.size(); s++)
	{
		if (Q_mesh[s] != 0) {
			for (int i = 0; i < lattice.size(); i++)
			{
				integrand[i] = iDensity[i] * sin(Q_mesh[s]*lattice.R(i)) / Q_mesh[s] / lattice.R(i);
			}

			Result[s] = I.Integrate(&integrand, 0, lattice.size() - 1);
		}
		else Result[s] = Q0;
	}
