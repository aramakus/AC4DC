	...
	for (int m = 0; m < adams_n; m++) {
		...
		while (error > tollerance) {
			...
			for (int a = 0; a < Store.size(); a++) {			
				double* p_A = A[a].data();
				double* p_X = X[a].data();
				double* p_Pht = Pht[a].data();
				double* p_ePht = ePht[a].data();
				double* p_Aug = Aug[a].data();
				double* p_eAug = eAug[a].data();
				double* p_S = S[a].data();
				double* p_eS = eS[a].data();
				double* p_Sp = Sp[a].data();
				double* p_eSp = eSp[a].data();
				double* p_W = W[a].data();

				vector<double*> * p_p = &map_p[a];
				vector<double*> * p_dpdt = &map_dpdt[a];
		
				for (auto& rate: Store[a].Photo) {
					tmp = rate.val*f[m];
					*(p_A + rate.from) -= tmp;
					*(p_Pht + rate.from) += tmp;
					*(p_ePht + rate.from) += tmp * rate.energy;
					*(p_X + rate.to) += tmp * *(p_p->at(rate.from) + m);
				}
				for (auto& rate: Store[a].Fluor) {
					*(p_A + rate.from) -= rate.val;
					*(p_X + rate.to) += rate.val * *(p_p->at(rate.from) + m);
				}
				for (auto& rate: Store[a].Auger) {
					*(p_A + rate.to) -= rate.val;
					*(p_X + rate.to) += rate.val * *(p_p->at(rate.from) + m);
				}
				...
				if (m > 0) {
					for (auto& EII: Store[a].EIIparams) {
						for (int i = 0; i < EII.fin.size(); i++) {
							...
							*(p_S + EII.init) += tmp;
							*(p_eS + EII.init) += tmp*EII.ionB[i];
							old_p = Factor * Elecs.sigmaBEB(e_t, EII.ionB[i], EII.kin[i], EII.occ[i]);
							*(p_Sp + EII.init) += old_p;
							*(p_eSp + EII.init) += old_p * EII.ionB[i];
							*(p_X + EII.fin[i]) += *(p_p->at(EII.init) + m) * ( old_p * Elecs.Np[m] + tmp * Elecs.N[m] );
							*(p_W + EII.init) += Factor * Elecs.sigmaBEBw1(e_t, EII.ionB[i], EII.kin[i], EII.occ[i]);
						}				
					}
				}
				...
				for (int i = 0; i < Store[a].num_conf; i++)
				{
					*(p_A + i) -= *(p_S + i) * Elecs.N[m] + *(p_Sp + i) * Elecs.Np[m];
					if (*(p_A + i) == 0 && *(p_X + i) == 0) continue;
					*(p_dpdt->at(i) + m) = *(p_A + i)* *(p_p->at(i) + m) + *(p_X + i);
					...
					*(p_X + i) = 0.;
					...
					{
						old_p = *(p_p->at(i) + m);
						*(p_p->at(i) + m) = *(p_p->at(i) + m - 1) + dt[m - 1]*0.5*(*(p_dpdt->at(i) + m) + *(p_dpdt->at(i) + m - 1));
						if (old_p == 0) tmp = fabs(*(p_p->at(i) + m) - old_p);
						else tmp = fabs(*(p_p->at(i) + m) - old_p) / *(p_p->at(i) + m);
						if (error < tmp) error = tmp;
					}

					tmp_dNdt += *(p_p->at(i) + m) * ( *(p_Aug + i) + Elecs.N[m]* *(p_S + i) + Elecs.Np[m]* *(p_Sp + i) );
					tmp_dEdt += *(p_p->at(i) + m) * ( *(p_eAug + i) - Elecs.N[m]* *(p_eS + i) + Elecs.Np[m]* *(p_W + i) );
					tmp_dNpdt += *(p_p->at(i) + m) * ( *(p_Pht + i) - Gesc*Elecs.Np[m] );
					tmp_dEpdt += *(p_p->at(i) + m) * ( *(p_ePht + i) - Gesc*Elecs.Ep[m] - Elecs.Np[m]*(*(p_eSp + i) + *(p_W + i)) );
				...
				// Correct plasma.
				Elecs.dNdt[m] += store.nAtoms * tmp_dNdt;
				Elecs.dEdt[m] += store.nAtoms * tmp_dEdt;
				Elecs.dNpdt[m] += store.nAtoms * tmp_dNpdt;
				Elecs.dEpdt[m] += store.nAtoms * tmp_dEpdt;
			}
...
int IntegrateRateEquation::Solve(Plasma & Elecs, vector<AtomRateData> & Store, int storage_time_pts)
{
	...
	for (int m = adams_n; m < t.size(); m++) {
		...
		e_t = Elecs.Ep[m] / Elecs.Np[m];
		v_t = sqrt(2*e_t);
		Gesc = 0;//1.5*v_t / store.R;
		Factor = 0;//v_t * 0.25 / Constant::Pi;	
		Temperature = 2*Elecs.E[m]/3/Elecs.N[m];
		Elecs.SetMaxwellPF(Temperature);

		for (int a = 0; a < Store.size(); a++) {
			tot_p_size = Store[a].num_conf;
			AtomRateData * p_Store = &Store[a];
			...
			double* p_eSp = eSp[a].data();
			double* p_W = W[a].data();
			double* p_Tbr = Tbr[a].data();
			double* p_eTbr = eTbr[a].data();
			...
			for (int i = 0; i < tot_p_size; i++) {
				...
				*(p_eTbr + i) = 0.;
				...
				// Predict p.
				tmp = *(p_p->at(i) + adams_n-1 );
				for (int j = 0; j < adams_n; j++) {
					tmp += Bashforth_5[j] * *(p_dpdt->at(i) + adams_n - j - 1) * dt[m - j - 1];
				}
				*(p_p->at(i) + adams_n) = tmp;
			}
			//predict dpdt
			for (auto& rate: p_Store->Photo) {
				tmp = rate.val * f[m];
				*(p_A + rate.from) -= tmp;
				*(p_Pht + rate.from) += tmp;
				*(p_ePht + rate.from) += tmp * rate.energy;
				*(p_X + rate.to) += tmp * *(p_p->at(rate.from) + adams_n);
			}
			for (auto& rate: p_Store->Fluor) {
				*(p_A + rate.from) -= rate.val;
				*(p_X + rate.to) += rate.val * *(p_p->at(rate.from) + adams_n);
			}
			for (auto& rate: p_Store->Auger) {
				*(p_A + rate.from) -= rate.val;
				*(p_X + rate.to) += rate.val * *(p_p->at(rate.from) + adams_n);
			}
			for (auto& EII: p_Store->EIIparams) {
				for (int i = 0; i < EII.fin.size(); i++) {
					tmp = 0*Elecs.MaxwellEII(EII.ionB[i], EII.kin[i], EII.occ[i]);
					mxw_tmp = 0*Pi3x8 * Elecs.MaxwellPF(EII.ionB[i]) * tmp;
					*(p_Tbr + EII.fin[i]) += mxw_tmp;
					*(p_eTbr + EII.fin[i]) += mxw_tmp * EII.ionB[i];
					*(p_X + EII.init) += mxw_tmp * *(p_p->at(EII.fin[i]) + adams_n) * Elecs.N[m] * Elecs.N[m];
					*(p_S + EII.init) += tmp;
					*(p_eS + EII.init) += tmp * EII.ionB[i];

					mxw_tmp = Factor * Elecs.sigmaBEB(e_t, EII.ionB[i], EII.kin[i], EII.occ[i]);
					*(p_Sp + EII.init) += mxw_tmp;
					*(p_eSp + EII.init) += mxw_tmp * EII.ionB[i];
					*(p_X + EII.fin[i]) += *(p_p->at(EII.init) + adams_n) * (mxw_tmp * Elecs.Np[m] + tmp * Elecs.N[m]);
					*(p_W + EII.init) += Factor * Elecs.sigmaBEBw1(e_t, EII.ionB[i], EII.kin[i], EII.occ[i]);
				}		
			}
			...
			tmp_dEpdt = 0;
			for (int i = 0; i < tot_p_size; i++) {// Free up last element in p[i] and erase the first one.
				*(p_A + i) -= Elecs.N[m] * (*(p_S + i) + Elecs.N[m] * *(p_Tbr + i) ) + Elecs.Np[m] * *(p_Sp + i); 
				tmp = *(p_A + i) * *(p_p->at(i) + adams_n) + *(p_X + i);
				*(p_dpdt->at(i) + adams_n) = tmp;
				tmp_dNpdt += *(p_p->at(i) + adams_n) * ( *(p_Pht + i) - Gesc * Elecs.Np[m] );
				tmp_dEpdt += *(p_p->at(i) + adams_n) * ( *(p_ePht + i) - Gesc * Elecs.Ep[m] - Elecs.Np[m] * (*(p_eSp + i) + *(p_W + i)) );
				tmp_dNdt += *(p_p->at(i) + adams_n) * ( *(p_Aug + i) + Elecs.N[m] * (*(p_S + i) - Elecs.N[m] * *(p_Tbr + i)) + Elecs.Np[m] * *(p_Sp + i) );
				tmp_dEdt += *(p_p->at(i) + adams_n) * ( *(p_eAug + i) - Elecs.N[m] * (*(p_eS + i) - Elecs.N[m] * *(p_eTbr + i)) + Elecs.Np[m] * *(p_W + i) );

				for (int j = 1; j < p_size; j++) {
					*(p_p->at(i) + j - 1) = *(p_p->at(i) + j);
					*(p_dpdt->at(i) + j - 1) = *(p_dpdt->at(i) + j);
				}
				*(p_p->at(i) + adams_n) = 0;
				*(p_dpdt->at(i) + adams_n) = 0;
			}
			Elecs.dNdt[m] += p_Store->nAtoms * tmp_dNdt;
			Elecs.dEdt[m] += p_Store->nAtoms * tmp_dEdt;
			Elecs.dNpdt[m] += p_Store->nAtoms * tmp_dNpdt;
			Elecs.dEpdt[m] += p_Store->nAtoms * tmp_dEpdt;

			for (int i = 0; i < tot_p_size; i++) {// Correct p.
				tmp = *(p_p->at(i) + adams_n - 2);
				for (int j = 0; j < adams_n; j++) {
					tmp += Moulton_5[j] * *(p_dpdt->at(i) + adams_n - j - 1) * dt[m - j];
				}
				*(p_p->at(i) + adams_n - 1) = tmp;
				if (tmp < 0 || tmp > 1) {
					printf("\n i = %d \n", i);
					return m;
				}
		...
		for (int a = 0; a < Store.size(); a++) {
			...
			for (auto& rate: p_Store->Photo) {
				...
				*(p_Pht + rate.from) += tmp;
				*(p_ePht + rate.from) += tmp * rate.energy;
				*(p_X + rate.to) += tmp * *(p_p->at(rate.from) + p_size - 2);
			}
			for (auto& rate: p_Store->Fluor) {
				*(p_A + rate.from) -= rate.val;
				*(p_X + rate.to) += rate.val * *(p_p->at(rate.from) + p_size - 2);
			}
			for (auto& rate: p_Store->Auger) {
				*(p_A + rate.from) -= rate.val;
				*(p_X + rate.to) += rate.val * *(p_p->at(rate.from) + p_size - 2);
			...
			for (auto& EII: p_Store->EIIparams) {
				for (int i = 0; i < EII.fin.size(); i++) {
					...
					*(p_Tbr + EII.fin[i]) += mxw_tmp;
					*(p_eTbr + EII.fin[i]) += mxw_tmp * EII.ionB[i];
					*(p_X + EII.init) += mxw_tmp * *(p_p->at(EII.fin[i]) + p_size - 2) * Elecs.N[m] * Elecs.N[m];
					*(p_S + EII.init) += tmp;
					*(p_eS + EII.init) += tmp * EII.ionB[i];
					...
					*(p_Sp + EII.init) += mxw_tmp;
					*(p_eSp + EII.init) += mxw_tmp * EII.ionB[i];
					*(p_X + EII.fin[i]) += *(p_p->at(EII.init) + p_size - 2) * (mxw_tmp * Elecs.Np[m] + tmp * Elecs.N[m]);
					*(p_W + EII.init) += Factor * Elecs.sigmaBEBw1(e_t, EII.ionB[i], EII.kin[i], EII.occ[i]);
				}		
			}
			...
			for (int i = 0; i < tot_p_size; i++) {// Free up last element in p[i] and erase the first one.
				*(p_A + i) -= Elecs.N[m] * (*(p_S + i) + Elecs.N[m] * *(p_Tbr + i) ) + Elecs.Np[m] * *(p_Sp + i); 
				tmp = *(p_A + i) * *(p_p->at(i) + p_size - 2) + *(p_X + i);
				*(p_dpdt->at(i) + p_size - 2) = tmp;
				tmp_dNpdt += *(p_p->at(i) + p_size - 2) * ( *(p_Pht + i) - Gesc * Elecs.Np[m] );
				tmp_dEpdt += *(p_p->at(i) + p_size - 2) * ( *(p_ePht + i) - Gesc * Elecs.Ep[m] - Elecs.Np[m] * (*(p_eSp + i) + *(p_W + i)) );
				tmp_dNdt += *(p_p->at(i) + p_size - 2) * ( *(p_Aug + i) + Elecs.N[m] * (*(p_S + i) - Elecs.N[m] * *(p_Tbr + i)) + Elecs.Np[m] * *(p_Sp + i) );
				tmp_dEdt += *(p_p->at(i) + p_size - 2) * ( *(p_eAug + i) - Elecs.N[m] * (*(p_eS + i) - Elecs.N[m] * *(p_eTbr + i)) + Elecs.Np[m] * *(p_W + i) );
			}
			Elecs.dNdt[m] += p_Store->nAtoms * tmp_dNdt;
			Elecs.dEdt[m] += p_Store->nAtoms * tmp_dEdt;
			Elecs.dNpdt[m] += p_Store->nAtoms * tmp_dNpdt;
