...
chem_elems = {'1' : 'H', '6' : 'C', '7' : 'N', '8' : 'O', '15' : 'P', '16' : 'S', '11' : 'Na', '17' : 'Cl'}

path = path.abspath(path.join(__file__ ,"../../output"))
File = open(path + "/MD_Data_NaCl.txt")
...
norm_max_chrg = max_chrg / np.max( max_chrg )
#norm_max_chrg = [0, 0.17, 0.44, 0.75, 1]
#norm_max_chrg = [0.7, 0.1]
...
File = open(path + "/Intensity_water_NaCl.txt")
# read the content into a list "Rates"
intsty = []
for line in File:
...
#ax2.set_ylim(bottom = 0, top = Temperature[-1]*1.2)
#ax2.set_ylabel('Temperature, eV')

plt.figlegend(loc = (0.11, 0.66))
